...
class Encoder(nn.Module):
    ...
    def encode_y(self, x, temperature, hard, adj_matrix):
        if self.block_type == "GCN":
            src, dst = np.nonzero(adj_matrix).T
            adj_matrix = dgl.graph((src, dst))
    ...
    def encode_z(self, x, y, adj_matrix):
        if self.block_type == "GCN":
            src, dst = np.nonzero(adj_matrix).T
            adj_matrix = dgl.graph((src, dst))
    ...
    def forward(self, x, adj_matrix, temperature=1.0, hard=0):
...
if __name__ == "__main__":
    ...
    gat_encoder = Encoder(y_dim=num_classes, z_dim=gaussian_size, in_channels=encoder_in_channels,
                          out_channels=encoder_out_channels
                          , block_type="Transformer", num_heads=4)
    gcn_encoder = Encoder(y_dim=num_classes, z_dim=gaussian_size, in_channels=encoder_in_channels,
                          out_channels=encoder_out_channels
                          , block_type="GCN", num_heads=4)

    y_gat_encoder, mu, var, z_gat_encoder, logits, prob = gat_encoder(x_genes, adj)
    print(y_gat_encoder.shape, logits.shape, prob.shape)
    print(mu.shape, var.shape, z_gat_encoder.shape)
    y_gcn_encoder, mu, var, z_gcn_encoder, logits, prob = gcn_encoder(x_genes, adj)
    print(y_gcn_encoder.shape, logits.shape, prob.shape)
    print(mu.shape, var.shape, z_gcn_encoder.shape)
